...
abstract class REST_Controller extends CI_Controller
{
	...
	/**
	 * If the request is authenticating succesfully based on the Authorization header provided. Store the $_authorizatoin_message in case in error
	 *
	 * @var boolean
	 */

        protected $_authorizatoin_message = NULL;
	/**
	 * If the request is authorized based on the authorization header provider.
	 *
	 * @var boolean
	 */
        protected $_is_authorized = TRUE;
	...
	*/
		...
		if (config_item('rest_enable_keys'))
		{
			$this->_allow = $this->_detect_api_key();                        
                ...
                if(!$this->_authenticate()){                    
                    //Unauthorized header                      
                    $this->_is_authorized = $this->_authenticate();                    
                }                
	...
	public function _remap($object_called, $arguments)
	{
		...
		if (config_item('rest_enable_keys') AND $use_key AND $this->_allow === FALSE)
		{
			...
			$this->response(array('status' => false, 'authenticated' => FALSE, 'error' => 'Invalid API Key.'), 403);
		}
                

                if($this->_is_authorized === FALSE){                                        
                                        
                    $response = array(
                        'status' => FALSE,
                        'authenticated' => FALSE,
                        'error' => $this->_authorizatoin_message
                    );                    

                    $this->response($response, $this->_response_code);  
	 ...
	 *
	 * compute a message to sign using some of http-header values to generate Authorization header
	 *
	 * @return boolean
	 */
        protected function _authenticate(){
            ...
            isset($this->rest->contentMD5) ? $this->rest->contentMD5 : "/n";
            ...
            $authenticate = self::_authorization_header($this->request->method, $this->rest->contentType, $this->rest->date, $this->rest->apiKey, $this->rest->resource, $this->rest->contentMD5);
            ...
            if($authenticate === $this->rest->authorization_header){
                ...
                if($diff > 5){
                    $this->_authorizatoin_message = 'Authorization Faild, Request Timeout'; 
                    $this->_response_code = '408'; 
                    return FALSE;
                }           
            ...
            $this->_authorizatoin_message = 'Authorization Faild, Bad Authorization Header Provided';
            $this->_response_code = '403'; 
            return FALSE;

        }

        protected function _authorization_header($verb, $contentType, $date, $apiKey, $canonicalizedResource, $contentMd5){
            
            $message = $verb . "\n" . $contentMd5 . "\n" . $contentType . "\n" . $date . "\n" . $apiKey . "\n" . $canonicalizedResource;
            $sign = base64_encode(hash_hmac('sha1', utf8_encode($message), $this->rest->secret_key));
            $authorization = "SAM " . $apiKey . ":" . $sign;
	...
	protected function _detect_api_key()
	{
		...
		if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
		{
			...
			isset($row->ignore_limits) AND $this->rest->ignore_limits = $row->ignore_limits;
                        isset($row->secret_key) AND $this->rest->secret_key = $row->secret_key;
