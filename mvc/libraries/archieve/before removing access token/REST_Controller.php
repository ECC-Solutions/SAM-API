...
abstract class REST_Controller extends CI_Controller
{
	...
	/**
	 * If the request is authenticating succesfully based on the Access token provided.
	 *
	 * @var boolean
	 */
	protected $_authenticated = FALSE;
        
        protected $_authorizatoin = NULL;

        protected $_extend_token_process = NULL;
	...
	*/
	...
	{
		parent::__construct();
                //check if compress is enabled in configuration
		$this->_zlib_oc = @ini_get('zlib.output_compression');

		// Lets grab the config and get ready to party
                //default response format, https .. etc
		...
		elseif (@$this->db)
		{
			$this->rest->db = $this->db;
                ...
                //Begain authenticating client
                //call the _authenticate()
                if(!$this->_authenticate()){
                    //Unauthorized header
                    $this->response($this->_authorizatoin, 401);
                }
                //$this->response($this->_authenticate(), 200);
		...
		if (config_item('rest_enable_keys'))
		{
			$this->_allow = $this->_detect_api_key();
                ...
                if(!$this->_detect_access_token())
                {                    
                    $this->_authenticated = $this->_detect_access_token();
                        
                }
                
                //checking the refresh token
                if($this->_extend_token() === false){
                    //checking to see if there an extending toking in the request
                    $this->_extend_token_process = $this->_extend_token();
                }
	...
	public function _remap($object_called, $arguments)
	{
		...
		if (config_item('rest_enable_keys') AND $use_key AND $this->_allow === FALSE)
		{
			...
			$this->response(array('status' => false, 'error' => 'Invalid API Key.'), 403);
                ...
                // Check to see if the extending token process made successful
                if($this->_extend_token_process === FALSE){
                    
                    $this->response('You have an error extending your access token, Authentication Failed', 401);                    
		...
		if (config_item('rest_enable_keys') AND !empty($this->rest->key))
		{
			...
			$authorized OR $this->response(array('status' => false, 'error' => 'This API key does not have enough permissions.'), 401);
                ...
                if($this->uri->segment('3') != 'login'){
                    if(empty($this->rest->access_token)){                   
                            $this->response(array('status' => false, 'authenticated' => false, 'error' => 'Authentication missing, Your authentication token expired, banned permanently or not provided'), 403);
                    }                    
                }
	...
	protected function _detect_method()
	{
        ...
        protected function _authenticate(){
            ...
            $this->rest->contentType = $this->input->get_request_header('Content-type');
            if($this->request->method === 'post' || $this->request->method === 'put' ){
                $this->rest->contentMD5 = $this->input->get_request_header('Content-md5');
            }
            $this->rest->authorization_header = $this->input->get_request_header('Authorization');
            $this->rest->date = $this->input->get_request_header('Date');
            $this->rest->apiKey = $this->input->get_request_header('X-sam-api-key');
            $this->rest->resource = $this->uri->uri_string();
            
            $authenticate = self::_authorization_header($this->request->method, $this->rest->contentMD5, $this->rest->contentType, $this->rest->date, $this->rest->apiKey, $this->rest->resource);                       
            
            if($authenticate === $this->rest->authorization_header){
                
                $diff = (strtotime(date('Y-m-d H:i:s', time())) - strtotime(date('Y-m-d H:i:s', strtotime($this->rest->date)))) / 60;                
                                
                if($diff > 5){
                    $this->_authorizatoin = 'Authorization Faild, Request Timeout';
                    return false;
                }
                return true;
            ...
            $this->_authorizatoin = 'Authorization Faild, Bad Authorization Header Provided';
            return false;            
            
        }

        protected function _authorization_header($verb, $contentMd5, $contentType, $date, $apiKey, $canonicalizedResource){
            
            $message = $verb . "\n" . $contentMd5 . "\n" . $contentType . "\n" . $date . "\n" . $apiKey . "\n" . $canonicalizedResource;
            $sign = base64_encode(hash_hmac('sha1', $message, '5cac3297f0d9f46e6ce1383881ba0980215cd71e'));
            $authorization = "Authorization: SAM " . $apiKey . ":" . $sign;
	...
	protected function _detect_api_key()
	{
        ...
        protected function _detect_access_token(){
                ...
                $this->rest->access_token = NULL;
		// Work out the name of the SERVER entry based on config
		$access_token = 'HTTP_'.strtoupper(str_replace('-', '_', 'x-sam-access-token'));
                
		// Find the access token from server or arguments
		if (($token = isset($this->_args['x-sam-access-token']) ? $this->_args['x-sam-access-token'] : $this->input->server($access_token)))
		{
			if ( ! ($row = $this->rest->db->where("tokenId_token = '" . $token . "' && valid_token = 1 && dateCreated_token + 3600 > '" . time() . "'")->get('ecc_tokens_token')->row()))
			{
                            
				return FALSE;
			}

			$this->rest->access_token = $row->tokenId_token;
                        $this->rest->scopes = $row->scopes_token;
                        $this->rest->token_identify = $row->userId_token;                        
                        
			return $row;
		}
                
                //$this->response(array('status' => false, 'error' => true, 'message' => 'Not authorized'), 403);
		// No TOKENS has been sent
		return FALSE;                  
                            
        }        
        
        protected function _extend_token(){
            
            $this->rest->extend_token = NULL;
            
            //Detect access and refresh
            $access_token = 'HTTP_'.strtoupper(str_replace('-', '_', 'x-sam-access-token'));            
            $token = isset($this->_args['x-sam-access-token']) ? $this->_args['x-sam-access-token'] : $this->input->server($access_token);
            
            
            $refresh_token = 'HTTP_'.strtoupper(str_replace('-', '_', 'x-sam-refresh-token'));            
            $extend = isset($this->_args['x-sam-refresh-token']) ? $this->_args['x-sam-refresh-token'] : $this->input->server($refresh_token);
            
            //Find refresh token from server or arguments
            if (!empty($extend) && !empty($token))            
            {
                $this->rest->db->where("refresh_token = '" . $extend . "' && valid_token = 1 && tokenId_token = '" . $token . "'")->set('dateCreated_token', function_exists('now') ? now() : time())->update('ecc_tokens_token');
                if ($this->db->affected_rows() === 1)
                {
                    
                      //token extedned successfully                        
                      return true;  
            ...
            }elseif(!empty($extend) && empty($token)){
                
                //token presented without the access_token
                return false;                                  
                
            }
            //extending token process not called yet
            return NULL;
